...
class TxLineClient:
    ...
    def scores_snapshot(self, fixture_id: str | int, *, as_of: int | None = None) -> Any:
        return self._json(
            f"/api/scores/snapshot/{self._segment(fixture_id)}",
            query={"asOf": int(as_of) if as_of is not None else None},
        )

    def score_stat_validation(
        self,
        *,
        fixture_id: str | int,
        seq: int,
        stat_key: int,
        stat_key2: int | None = None,
    ) -> Any:
        fixture_text = str(fixture_id).strip()
        if not fixture_text:
        ...
        return self._json(
            ...
            query={
                "fixtureId": fixture_text,
                "seq": int(seq),
                "statKey": int(stat_key),
                "statKey2": int(stat_key2) if stat_key2 is not None else None,
            },
        )

    def _headers(self) -> dict[str, str]:
        headers = {"Accept": "application/json", "User-Agent": "finalitygate-worldcup-resolver/0.1"}
