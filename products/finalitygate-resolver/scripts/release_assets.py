...
SBOM = DIST / "finalitygate-worldcup-resolver.spdx.json"
JUDGE_PACK = DIST / "finalitygate-worldcup-resolver-judge-pack.zip"
FIXED_ZIP_TIME = (2026, 1, 1, 0, 0, 0)

EXCLUDED_PARTS = {".git", ".venv", "__pycache__", ".pytest_cache", "dist", "build", "outputs"}
TEXT_SUFFIXES = {".py", ".md", ".toml", ".json", ".txt", ".ps1", ".sh", ".html", ".example", ".gitignore", ".dockerignore"}
...
def generate_sbom(entries: list[FileEntry]) -> dict[str, Any]:
    ...
    payload = {
        ...
        "name": "finalitygate-worldcup-resolver-0.1.0",
        "documentNamespace": "https://example.invalid/spdx/finalitygate-worldcup-resolver/0.1.0",
        "creationInfo": {"creators": ["Tool: finalitygate-release-assets-v1"]},
        "packages": [{"name": "finalitygate-worldcup-resolver", "SPDXID": "SPDXRef-Package", "versionInfo": "0.1.0", "downloadLocation": "NOASSERTION", "filesAnalyzed": True, "licenseConcluded": "MIT", "licenseDeclared": "MIT"}],
        "files": files,
...
def build_judge_pack(entries: list[FileEntry]) -> dict[str, Any]:
    ...
    with zipfile.ZipFile(JUDGE_PACK, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
        ...
        manifest_data = (json.dumps({"schema": "finalitygate.judge-pack-manifest.v1", "files": manifest_entries}, indent=2) + "\n").encode("utf-8")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build FinalityGate release security evidence, SPDX SBOM, and deterministic judge pack")
