...
VALIDATION_FIXTURE = ROOT / "tests" / "fixtures" / "valid_odds_validation.json"
...
def _run(name: str, argv: list[str], *, cwd: Path = ROOT, timeout: int = 900, env: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "duration_seconds": round(time.perf_counter() - started, 3),
            "command": argv,
            "stdout_tail": _tail(exc.stdout.decode("utf-8", errors="replace") if isinstance(exc.stdout, bytes) else exc.stdout),
            "stderr_tail": _tail(exc.stderr.decode("utf-8", errors="replace") if isinstance(exc.stderr, bytes) else exc.stderr),
        }
...
def _contract(name: str, passed: bool, detail: str) -> dict[str, Any]:
    return {
        ...
        "command": ["internal-contract-check"],
        "stdout_tail": detail if passed else "",
        "stderr_tail": "" if passed else detail,
    }
...
def run_gate(timeout: int) -> dict[str, Any]:
    ...
    def execute(name: str, argv: list[str], *, cwd: Path = ROOT, env: dict[str, str] | None = None) -> bool:
        print(f"[proofguard-local] {name}", flush=True)
    ...
    with tempfile.TemporaryDirectory(prefix="proofguard-release-") as temp_name:
        temp = Path(temp_name)
        demo_a = temp / "demo-a"
        demo_b = temp / "demo-b"
        validation_inspection = temp / "odds-validation-inspection.json"
        commands = [
            ("compile", [sys.executable, "-m", "compileall", "-q", "src", "tests", "scripts"]),
            ("collect-tests", [sys.executable, "scripts/count_tests.py"]),
            ("pytest", [sys.executable, "-m", "pytest", "-q"]),
            ("doctor", [sys.executable, "-m", "proofguard_agent", "doctor"]),
            ("web-smoke", [sys.executable, "-m", "proofguard_agent.web.smoke"]),
            (
                "inspect-official-odds-validation-fixture",
                [
                    sys.executable,
                    "-m",
                    "proofguard_agent",
                    "inspect-odds-validation",
                    "--input",
            ...
            ("demo-a", [sys.executable, "-m", "proofguard_agent", "demo", "--out", str(demo_a)]),
            ("verify-demo-a", [sys.executable, "-m", "proofguard_agent", "verify-demo", str(demo_a)]),
            ("demo-b", [sys.executable, "-m", "proofguard_agent", "demo", "--out", str(demo_b)]),
            ("verify-demo-b", [sys.executable, "-m", "proofguard_agent", "verify-demo", str(demo_b)]),
        ]
        ...
        if len(steps) == len(commands) and all(step["status"] == "PASS" for step in steps):
            hash_a = _digest_tree(demo_a)
            hash_b = _digest_tree(demo_b)
        ...
        if package_ok and all(step["status"] == "PASS" for step in steps):
            wheel_target = temp / "wheel-install"
            wheel_target.mkdir()
            with zipfile.ZipFile(wheels[0]) as archive:
                archive.extractall(wheel_target)
            required = {
                "proofguard_agent/core.py",
                "proofguard_agent/cli.py",
                "proofguard_agent/demo.py",
                "proofguard_agent/adapter.py",
                "proofguard_agent/txline.py",
                "proofguard_agent/validation.py",
                "proofguard_agent/web/__init__.py",
                "proofguard_agent/web/app.py",
                "proofguard_agent/web/runtime.py",
                "proofguard_agent/web/smoke.py",
            }
            ...
            if not missing:
                ...
                wheel_validation = temp / "wheel-odds-validation.json"
                clean_env = os.environ.copy()
                clean_env["PYTHONPATH"] = str(wheel_target)
                clean_env["PYTHONNOUSERSITE"] = "1"
                clean_env["PROOFGUARD_MODE"] = "REPLAY"
                clean_env["PROOFGUARD_AUTO_START"] = "false"
                execute("isolated-wheel-doctor", [sys.executable, "-m", "proofguard_agent", "doctor"], cwd=temp, env=clean_env)
                execute("isolated-wheel-web-smoke", [sys.executable, "-m", "proofguard_agent.web.smoke"], cwd=temp, env=clean_env)
                execute(
                    "isolated-wheel-odds-validation-inspection",
                    [
                        sys.executable,
                        "-m",
                        "proofguard_agent",
                ...
                execute("isolated-wheel-demo", [sys.executable, "-m", "proofguard_agent", "demo", "--out", str(wheel_demo)], cwd=temp, env=clean_env)
                execute("isolated-wheel-verify", [sys.executable, "-m", "proofguard_agent", "verify-demo", str(wheel_demo)], cwd=temp, env=clean_env)
        ...
        checks = (summary_a or {}).get("checks", {})
        ...
        if all(step["status"] == "PASS" for step in steps):
            ...
            steps.append(_contract("autonomy-contract", isinstance(checks, dict) and bool(checks) and all(checks.values()), f"checks={checks}"))
            steps.append(_contract(
                "official-odds-validation-structure",
                validation_result.get("status") == "PASS"
                and validation_result.get("exact_leaf_serialization_executed") is False
                and validation_result.get("onchain_validate_odds_executed") is False,
                f"status={validation_result.get('status')}; exact_leaf_serialization_executed={validation_result.get('exact_leaf_serialization_executed')}; onchain_validate_odds_executed={validation_result.get('onchain_validate_odds_executed')}",
            ))
        ...
        release = {
            ...
            "autonomy_checks": checks,
            "unsafe_entry_count": (summary_a or {}).get("unsafe_entry_count"),
            ...
            "official_odds_validation_structure_checked": validation_result.get("status") == "PASS",
            "odds_validation_structural_fingerprint_sha256": validation_result.get("structural_fingerprint_sha256"),
            ...
            "live_adapter_packaged": wheel_files_passed,
            "live_web_packaged": wheel_files_passed,
            ...
            "claim_boundary": "live or replay-driven autonomous simulated paper execution with explicit source labelling; no real-money betting, custody, crypto-asset execution, profitability claim, exact leaf serialization claim, or on-chain validateOdds claim",
        }
    ...
    return {
        "schema": "proofguard.local-validation-report.v2",
        "status": "PASS" if steps and all(step["status"] == "PASS" for step in steps) else "FAIL",
        "commit": _git_commit(),
        "python": sys.version,
        ...
        "notes": [
            ...
            "The public service labels LIVE, REPLAY, REPLAY_FALLBACK, and ERROR explicitly.",
            "Offline official-payload inspection is not equivalent to exact leaf serialization or executing Solana validateOdds.",
        ],
    }


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run ProofGuard validation locally without GitHub Actions")
